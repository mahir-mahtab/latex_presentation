...
\begin{canvas}
  ...
  \node at (10.35,4.75) {\includegraphics[width=2.2cm]{frame_01}};
  ...
  \node[anchor=north west,note,text width=3.0cm,visible on=<2-3>] at (11.9,4.7)
       {Now slide $z$ along the line towards $z_b$ \dots};
...
\begin{canvas}
  ...
  \node[font=\footnotesize,text=slate] at (5.02,6.3) {\dots};
  \node[anchor=west,font=\scriptsize,text=slate] at (5.4,6.3) {random digit};
  ...
  \node[anchor=north,font=\scriptsize,text=steel,visible on=<3->] at (3.85,3.13) {sees the image \emph{and} its label};
  \node[anchor=north,font=\scriptsize,text=teal!80!black,visible on=<3->] at (10.9,3.13) {draws the digit $y$ asks for};
  % ---------- step 4: the intuition ----------
  \node[mathbox,anchor=south west,text width=7.9cm,minimum height=1.3cm,align=left,font=\footnotesize,visible on=<4->] at (1.1,0.98)
       {The label $y$ says \textbf{\textcolor{teal!80!black}{which}} digit.
        \\[2pt] So $z$ only stores \textbf{\textcolor{navy}{how}} it is written.};
  % ---------- step 5: the payoff ----------
  \begin{scope}[visible on=<5->]
    \node[keybox,anchor=south west,text width=3.3cm,minimum height=1.3cm,align=left,font=\footnotesize] at (9.45,0.98)
         {$y=7$: \textbf{always a 7}, only the \emph{style} varies.};
    \foreach \f/\xx in {7_l/13.55, 7_m/14.05, 7_r/14.55}
...
\begin{scope}[shift={(0,-0.25)}]
  ...
  \begin{scope}[visible on=<2->]
    ...
    \node[chip,draw=slate!70,text=ink,font=\scriptsize,inner ysep=2pt] (prompt) at (7.44,1.72) {``a smiling face''};
...
% =================================================================
%  CLOSING SLIDE — title-slide style
% =================================================================
{
\setbeamertemplate{background}{}
\setbeamertemplate{footline}{}
...
\begin{tikzpicture}[remember picture,overlay,shift={(current page.south west)}]
  ...
  \begin{scope}
    ...
    % the walk from Part 3, drawn through the medallion
    \draw[white,line width=0.9pt,dash pattern=on 3pt off 2pt] ($(C)+(-0.8,0.6)$) -- ($(C)+(0.9,0.4)$);
    \fill[white] ($(C)+(-0.8,0.6)$) circle (0.07) ($(C)+(0.9,0.4)$) circle (0.07);
  \end{scope}
  \node[font=\scriptsize,text=steel] at ($(C)+(0,-3.0)$) {compress $\rightarrow$ smooth $\rightarrow$ sample};
  \node[anchor=west,font=\small\bfseries,text=teal] at (1.0,6.55) {CSE 200 \;\textbullet\; B2 \;\textbullet\; Group 2};
  \node[anchor=north west,text=navy,align=left,font=\fontsize{30}{33}\fontseries{eb}\selectfont]
       at (0.9,6.2) {THANK YOU};
  \fill[teal] (1.0,4.6) rectangle ++(1.6,0.12);
  \fill[sky]  (2.75,4.6) rectangle ++(0.45,0.12);
  \node[anchor=west,text=steel,font=\large] at (0.95,4.05) {Questions?};
  \node[anchor=west,text=ink,font=\small,align=left] at (1.0,3.05)
       {Generate \,\textbullet\, interpolate \,\textbullet\, condition \,\textbullet\, compress};
