...
\begin{tikzpicture}[scale=0.86,every node/.append style={transform shape},
  ...
  % ---------- decoder ----------
  \node[block,shape border rotate=90,fill=clay!10,draw=clay,text=clay!70!black,
  ...
  % ---------- loss braces ----------
  \draw[visible on=<5->,decorate,decoration={brace,amplitude=6pt,mirror},clay,line width=0.8pt]
       (0,-2.45) -- (13.99,-2.45)
       node[midway,below=7pt,text=clay!80!black,font=\footnotesize] {reconstruction: make $\hat{x}\approx x$};
...
\only<5>{%
  \begin{beamercolorbox}[rounded=true,wd=0.8\linewidth,center,sep=4pt]{block body}
    ...
    $\mathcal{L}(\theta,\phi;x)=
      \underbrace{\mathbb{E}_{q_\phi(z\mid x)}\!\left[\log p_\theta(x\mid z)\right]}_{\color{clay}\text{reconstruction}}
      \;-\;
