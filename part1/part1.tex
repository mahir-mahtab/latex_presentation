...
\tikzset{
  ...
  emphasis on/.style={alt={#1{draw=plum,line width=1.3pt,fill=plum!4}{}}},
}
...
\begin{canvas}
  ...
  \only<2->{\node[anchor=east,font=\footnotesize,text=slate] at (14.6,4.12)
    {127.6 GB used \hspace{0.35cm} \textcolor{clay}{\textbf{only 0.4 GB free}}};}
...
\begin{canvas}
  ...
  \node[anchor=north west,text width=6.2cm,align=left,font=\normalsize,text=ink,visible on=<3->] at (8.95,3.5) {%
    Large \textbf{\textcolor{clay}{empty gaps}} remain between the islands. The decoder has not seen training examples from these regions.};
\end{canvas}
\end{frame}
...
\begin{canvas}
  ...
  \begin{scope}[visible on=<3->]
    ...
    \node[anchor=north west,text width=5.85cm,align=left,font=\footnotesize,text=ink] at (9.08,2.39) {%
      ...
      \textbf{\textcolor{clay}{That is the central idea behind a VAE.}}};
  \end{scope}
\end{canvas}
\end{frame}
