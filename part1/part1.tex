...
\begin{tikzpicture}[remember picture,overlay,shift={(current page.south west)}]
  ...
  \node[anchor=west,font=\small\bfseries,text=teal] at (1.0,6.55) {CSE 200 \;\textbullet\; B2 \;\textbullet\; Group 2};
...
\begin{canvas}
  ...
  \begin{scope}
    ...
    \node[anchor=north west,inner sep=0pt] at (1.58,6.36) {\includegraphics[width=2.94cm]{gallery.png}};
  ...
  \node[anchor=north west,text width=8.6cm,align=left,font=\normalsize,text=ink] at (5.9,5.55)
       {Imagine your phone has \textbf{20,000 photos}, but the storage is \textcolor{teal}{\textbf{almost full}}.};

  % storage bar
  \node[anchor=west,font=\footnotesize\bfseries,text=ink] at (5.9,4.12) {Storage};
  \only<1>{\node[anchor=east,font=\footnotesize,text=slate] at (14.6,4.12)
  ...
  \end{scope}
  \draw[teal,line width=1.3pt,rounded corners=2pt,visible on=<2>] (5.9,3.45) rectangle (14.6,3.85);
  ...
  \begin{scope}[visible on=<3->]
    ...
    \draw[teal,line width=1.1pt,rounded corners=4pt,fill=white] (10.45,1.15) rectangle (14.6,2.55);
    \node[anchor=west,font=\Large,text=teal] at (10.65,1.85) {\cmark};
    \node[anchor=west,align=left,text width=3.15cm,font=\footnotesize,text=ink] at (11.2,1.85)
         {\textbf{\textcolor{teal}{A smarter solution}}\\{\scriptsize keep every photo, but use far less space}};
  \end{scope}
\end{canvas}
\end{frame}
...
\begin{canvas}
  ...
  \begin{scope}
    ...
    \foreach \p in {fa,fb}{
      ...
      \end{scope}}
    \node[mystery,minimum width=1.5cm,minimum height=0.9cm,font=\Large\fontseries{eb}\selectfont,emphasis on=<3->]
         (fm) at (8,6.6) {?};
    \draw[arr] (fa) -- (fm);
    \draw[arr] (fm) -- (fb);
  \end{scope}
  ...
  % ---------- real photos ----------
  \only<2->{\begin{scope}
    \node[inner sep=0pt] (po) at (3.9,3.95) {\includegraphics[width=4.5cm]{photo_original.jpg}};
    \draw[photo] (po.south west) rectangle (po.north east);
    \node[inner sep=0pt] (pc) at (12.1,3.95) {\includegraphics[width=4.5cm]{photo_compressed.jpg}};
    \draw[photo] (pc.south west) rectangle (pc.north east);
    \node[mystery,minimum width=1.9cm,minimum height=1.5cm,emphasis on=<3->] (pm) at (8,3.95) {?};
    \draw[arr] (po) -- (pm);
    \draw[arr] (pm) -- (pc);
  ...
  \only<3->{\node[font=\normalsize,text=ink,align=center] at (8,1.3)
       {Same photo, much less storage. \; What goes inside the \textbf{\textcolor{deepteal}{mysterious middle box}}?};}
\end{canvas}
\end{frame}
...
\begin{canvas}
  \node[inner sep=0pt] (po) at (3.3,4.5) {\includegraphics[width=3.9cm]{photo_original.jpg}};
  \draw[photo] (po.south west) rectangle (po.north east);
  \node[inner sep=0pt] (pc) at (12.7,4.5) {\includegraphics[width=3.9cm]{photo_compressed.jpg}};
  \draw[photo] (pc.south west) rectangle (pc.north east);
  ...
  \node[visible on=<3->,lbl] at (8,1.45)
       {a neural network that learns to \textcolor{steel}{\textbf{compress}} an image and \textcolor{teal}{\textbf{rebuild}} it};
\end{canvas}
\end{frame}
...
\begin{canvas}
  ...
  \begin{scope}[visible on=<1->]
    ...
    \node[inner sep=0pt] (xin) at (2.0,\yc) {\includegraphics[width=2.1cm]{photo_original.jpg}};
  ...
  \begin{scope}[visible on=<3->]
    ...
    \node[inner sep=0pt] (xout) at (13.9,\yc) {\includegraphics[width=2.1cm]{photo_compressed.jpg}};
  ...
  \node[font=\normalsize,text=ink,align=center,text width=13cm] at (8,1.55) {%
    ...
    \only<3>{The \textbf{\textcolor{teal}{decoder}} uses that compact code to rebuild the photo: \; $\hat{x}\approx x$.}};
\end{canvas}
\end{frame}
...
\begin{canvas}
  \begin{scope}[shift={(4.65,4.1)}]
    ...
    \begin{scope}[visible on=<2->]
      \draw[teal,line width=0.9pt] (2.2,1.2) circle (0.14);
      \draw[teal,line width=0.6pt] (2.33,1.28) -- (3.1,2.4);
      \node[fill=white,draw=teal,line width=0.6pt,rounded corners=2pt,font=\scriptsize,text=teal!80!black,
  ...
  \node[anchor=north west,text width=6.2cm,align=left,font=\normalsize,text=ink,visible on=<3->] at (8.95,3.5) {%
    Large \textbf{\textcolor{teal}{empty gaps}} remain between the islands. The decoder has not seen training examples from these regions.};
\end{canvas}
\end{frame}
...
\begin{canvas}
  \begin{scope}[shift={(4.65,4.1)}]
    ...
    \begin{scope}[visible on=<2->]
      ...
      \node[star,star points=5,star point ratio=2.3,fill=teal,inner sep=1.4pt] at (-1.25,0.3) {};
      \node[font=\scriptsize\bfseries,text=teal] at (-1.25,0.95) {B};
    \end{scope}
  \end{scope}
  ...
  \begin{scope}[visible on=<2->]
    \node[font=\footnotesize,text=teal!80!black,anchor=west] at (8.95,4.6) {\textbf{B}\; random point from an empty gap};
    \node[smalldec] (dB) at (11.2,3.6) {Decoder};
    \draw[arr] (9.3,3.6) -- (dB.west);
    \garbled{13.25,3.6}{1.1}
    \draw[arr] (dB.east) -- (12.65,3.6);
    \node[font=\footnotesize,text=teal!80!black,anchor=north] at (13.25,2.98) {\xmark\ meaningless};
  \end{scope}

  % idea box
  \begin{scope}[visible on=<3->]
    \draw[fill=slate!7,draw=navy!40,line width=0.6pt,rounded corners=4pt] (8.95,0.98) rectangle (15.05,2.5);
    \node[anchor=north west,text width=5.85cm,align=left,font=\footnotesize,text=ink] at (9.08,2.39) {%
      Suppose the encoder maps each image to a \textbf{\textcolor{navy}{distribution}} instead of one point.\\[3pt]
      \textbf{\textcolor{teal}{That is the central idea behind a VAE.}}};
  \end{scope}
\end{canvas}
\end{frame}
...
\begin{canvas}
  ...
  \node[chip,draw=teal,anchor=west,text width=5.55cm,inner xsep=5pt,align=left,visible on=<2->,line width=1.1pt] at (9.1,3.3)
  ...
  \node[anchor=north west,text width=5.9cm,align=left,font=\footnotesize,text=ink] at (9.15,2.55) {%
    ...
    \only<3->{The distributions overlap, which \textbf{reduces empty gaps}. Nearby samples are more likely to decode into realistic images.}};
\end{canvas}
\end{frame}
...
\begin{canvas}
  ...
  \begin{scope}[visible on=<2->]
    ...
    \node[lbl] at (8.0,1.75) {illustrative latent attributes: one distribution each};
  \end{scope}
...
\begin{canvas}
  \node[anchor=west,font=\footnotesize,text=slate] at (0.95,6.95)
       {So far: \; \textcolor{steel}{\textbf{Autoencoder}} = one point per image \quad\textbullet\quad
                   \textcolor{teal}{\textbf{VAE}} = one distribution per image};
  \begin{scope}[shift={(1.65,4.55)},scale=0.9,every node/.append style={transform shape},
    ...
    \begin{scope}[visible on=<2->]
      \foreach \xa/\xb/\n/\c in {1.85/6.1/1/steel, 6.45/9.55/2/navy, 9.75/12.2/3/teal}{
  ...
  \begin{scope}[visible on=<2->]
    ...
    \node[font=\small,text=ink] at (8,1.575)
      {Handing over to \textbf{\textcolor{navy}{Member 2}}: \;
       \textcolor{steel}{\ding{172} encoder} $\rightarrow$ \textcolor{navy}{\ding{173} sampling $z$} $\rightarrow$ \textcolor{teal!80!black}{\ding{174} decoder}};
  \end{scope}
\end{canvas}
\end{frame}
